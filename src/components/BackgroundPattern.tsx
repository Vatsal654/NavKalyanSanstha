...
interface BackgroundPatternProps {
  ...
  position?: string; // e.g., 'top-0 left-1/2', 'bottom-0 right-0'
}
...
const BackgroundPattern: React.FC<BackgroundPatternProps> = ({
  ...
  opacity = 'opacity-50', // Increased opacity for better visibility
  position = 'absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2', // Centered by default
}) => {
  return (
    <div className={`absolute inset-0 overflow-hidden pointer-events-none ${className}`}>
      <Icon
        className={`text-primary ${opacity} ${position}`} // Changed color to text-primary
